...
#         super().__init__()
#         self.dw = nn.Conv2d(c1, c1, k, s, padding=k//2, groups=c1)
#         self.point = Conv(c1, c2, 1, 1, act=False)
#         self.dw2 = Conv(c2, c2, 3, 1, 1, c2, act=False)
#         self.act = nn.SiLU()

#     def forward(self, x):
#         y = self.point(self.dw(x))

#         return self.act(self.dw2(y) + y)

    
class DSPconv(nn.Module):
    def __init__(self, c1, c2, k, s, n_div=2):
        super().__init__()
        self.c_ = c1 // n_div
        self.c_2 = c2 // n_div
        self.dsconv = nn.Sequential(
            Conv(self.c_, self.c_, k, s, p=k//2, g=self.c_),
            Conv(self.c_, self.c_2, 1, 1)
        )

        self.conv = Conv(self.c_, self.c_2, k, s)
...
class PDconv(nn.Module):
    ...
    def __init__(self, c1, c2, k, s, n_div=2):
        ...
        self.pointconv = Conv(self.c_ + self.c_2, c2, 1, 1)
    ...
    def forward(self, x):
        ...
        return self.pointconv(torch.cat([x1, x2], dim=1))
...
class DSConv2(nn.Module):
    def __init__(self, c1, c2, k=1, s=1):
        super().__init__()
        self.dw = Conv(c1, c1, k, s, k//2, c1, act=False)
        self.dw2 = Conv(c1, c1, 5, 1, 2, c1, act=False)
        self.point = Conv(c1, c2, 1, 1)

    def forward(self, x):
        y = self.dw(x)

        return self.point((self.dw2(y) + y))
...
class MFC(nn.Module):
    def __init__(self, c1, h, w, k=3, s=1, attn=False):
        super().__init__()
        ...
        self.get_h = Conv(w+c1, 1, 1, 1)
        self.get_w = Conv(h+c1, 1, 1, 1)
        self.get_c = Conv(h+w, 1, 1, 1)

        self.sig = nn.Sigmoid()

    def forward(self,x):
        b, c, h, w = x.shape
        hw = self.hw(x)          #b,1,h,w

        cw = self.wc(x.permute(0, 2, 1, 3)).permute(0, 2, 1, 3)   # (x.transpose(1, 2)).transpose(1, 2) #b,c,1,w
        hc = self.hc(x.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)   # (x.transpose(1, 3)).transpose(1, 3) #b,c,h,1

        vec_h = self.get_h(torch.cat([hw.permute(0, 3, 2, 1), hc],dim=1)) #b,1,h,1
        vec_w = self.get_w(torch.cat([hw.permute(0, 2, 1, 3), cw],dim=1)) #b,1,1,w
        vec_c = self.get_c(torch.cat([cw.permute(0, 3, 2, 1), hc.permute(0, 2, 3, 1)], dim=1)).permute(0, 3, 2, 1) #b,c,1,1

        weight = [self.sig(vec_h.mean()), self.sig(vec_w.mean()), self.sig(vec_c.mean())]
        mean = (weight[0] + weight[1] + weight[2]) / 3

        expanded_vh = torch.full(size=(b, c, h, w), fill_value=float(0), device=vec_h.device, dtype=vec_h.dtype)
        expanded_vh[:, 0:1, :, 0:1] = vec_h

        expanded_vw = torch.full(size=(b, c, h, w), fill_value=float(0), device=vec_w.device, dtype=vec_w.dtype)
        expanded_vw[:, 0:1, 0:1, :] = vec_w

        expanded_vc = torch.full(size=(b, c, h, w), fill_value=float(0), device=vec_c.device, dtype=vec_c.dtype)
        expanded_vc[:, :, 0:1, 0:1] = vec_c

        out = (torch.exp(mean - weight[0]) * expanded_vh + torch.exp(mean - weight[1]) * expanded_vw + torch.exp(mean - weight[2]) * expanded_vc) + x
        return out