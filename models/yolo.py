...
class Decoupled_Detect(nn.Module):
    ...
    def __init__(self, nc=6, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        #     nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 5 * self.na, 1)) for x in ch
        # )
        # self.cv3 = (nn.ModuleList(
        #                 nn.Sequential(
        #                     nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)),
        #                     nn.Sequential(DWConv(c3, c3, 3), Conv(c3, c3, 1)),
        #                     nn.Conv2d(c3, self.nc * self.na, 1),
        #                 ) for x in ch
        #         )
        # )

        self.cv2 = nn.ModuleList(
            nn.Sequential(DWConv(x, x, 3), nn.Conv2d(x, 5 * self.na, 1)) for x in ch
        )
        
        self.cv3 = nn.ModuleList(
            nn.Sequential(DWConv(x, x, 3), nn.Conv2d(x, self.nc * self.na, 1)) for x in ch
        )
...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            if visualize:# and m.type == 'models.common.C3_triple3':
                feature_visualization_all(x, m.type, m.i, save_dir=visualize)
        return x

    def _profile_one_layer(self, m, x, dt):
        """Profiles a single layer's performance by computing GFLOPs, execution time, and parameters."""
        c = isinstance(m, (Detect, Segment, Decoupled_Detect))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", type=str, default="/home/jndx01/yang/yolov5-master/models/test/yolov11.yaml", help="model.yaml")
