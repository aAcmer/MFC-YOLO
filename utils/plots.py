...
def plot_featuremap(x, hw, cw, hc, map_h, map_w, map_c, out):
    b,channels,h,w =x.size()
    if h>=80:
        f0 = "/home/jndx01/yang/yolov5-master/featuremap/xy/" + f"input_features_{h}.png"  # filename
        ...
        for i in range(n):
            ax[i].imshow(blocks[i].squeeze())  # cmap='gray'
            ax[i].axis("off")
        plt.savefig(f0, dpi=300, bbox_inches="tight")

        f1 = "/home/jndx01/yang/yolov5-master/featuremap/2/" + f"hw_features_{h}.png"  # filename
        os.makedirs(os.path.dirname(f1), exist_ok=True)
        img = hw[0].cpu().squeeze().detach().numpy()
        plt.imsave(f1, img)

        f2 = "/home/jndx01/yang/yolov5-master/featuremap/2/" + f"cw_features_{h}.png"  # filename
        img = cw[0].cpu().squeeze().detach().numpy()
        plt.imsave(f2, img)

        f3 = "/home/jndx01/yang/yolov5-master/featuremap/2/" + f"hc_features_{h}.png"  # filename
        img = hc[0].cpu().squeeze().detach().numpy()
        plt.imsave(f3, img)

        f4 = "/home/jndx01/yang/yolov5-master/featuremap/1/" + f"h_features_{h}.png"  # filename
        os.makedirs(os.path.dirname(f4), exist_ok=True)
        blocks = torch.chunk(map_h[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
        n = min(32, channels)  # number of plots
        fig, ax = plt.subplots(math.ceil(n / 8), 8, tight_layout=True)  # 8 rows x n/8 cols
        ax = ax.ravel()
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        ...
        f5 = "/home/jndx01/yang/yolov5-master/featuremap/1/" + f"w_features_{h}.png"  # filename
        blocks = torch.chunk(map_w[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
        n = min(32, channels)  # number of plots
        fig, ax = plt.subplots(math.ceil(n / 8), 8, tight_layout=True)  # 8 rows x n/8 cols
        ax = ax.ravel()
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        ...
        f6 = "/home/jndx01/yang/yolov5-master/featuremap/1/" + f"c_features_{h}.png"  # filename
        blocks = torch.chunk(map_c[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
        n = min(32, channels)  # number of plots
        fig, ax = plt.subplots(math.ceil(n / 8), 8, tight_layout=True)  # 8 rows x n/8 cols
        ax = ax.ravel()
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        ...
        f7 = "/home/jndx01/yang/yolov5-master/featuremap/xy/" + f"out_features_{h}.png"  # filename
        blocks = torch.chunk(out[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
        n = min(32, channels)  # number of plots
        fig, ax = plt.subplots(math.ceil(n / 8), 8, tight_layout=True)  # 8 rows x n/8 cols
        ax = ax.ravel()
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
